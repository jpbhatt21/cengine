...
let fiftyMove = 0;
let moves=0;
export function setData(chk:boolean,enp:number[], cast:boolean[], fif:number, mov:number){
	check=chk;
	enpassant=enp;
	castling=cast;
	fiftyMove=fif;
...
export function moveManager(k: any, board: any, turn: boolean) {
  ...
  let enemyPieces = boardMatrix.map((x) =>
    x.map((y: any) =>
      (turn ? y.toLowerCase() : y.toUpperCase()) == y && y != "-" ? true : false
    )
  );
...
function pawnMoves(
  ...
  if (i + direction < 8 && empty[i + direction][j]) {
    ...
    if (!main || !checkForCheck(flatten(temp), turn)) {
      ...
      if (
        ((i == 1 && direction == 1) ||
        (i == 6 && direction == -1)) && empty[i + 2 * direction][j]
      ) {
  ...
  if (j + 1 < 8 && i + direction < 8 && (enemy[i + direction][j + 1]||enpassant[0]==i&&enpassant[1]==j+1)) {
  ...
  temp = deepCopy(board);
  if (j - 1 >= 0 && i + direction < 8 && (enemy[i + direction][j - 1]||enpassant[0]==i&&enpassant[1]==j-1)) {
    temp[i + direction][j - 1] = temp[i][j];
    temp[i][j] = "-";
    if (!main || !checkForCheck(flatten(temp), turn))
      moves.push((i + direction) * 8 + j - 1);
...
function knightMoves(
  ...
  for (let x of directions) {
    ...
    if (i2 >= 0 && i2 < 8 && j2 >= 0 && j2 < 8 && moveable[i2][j2]) {
      ...
      temp[i2][j2] = temp[i][j];
      temp[i][j] = "-";
      if (!main || !checkForCheck(flatten(temp), turn)) moves.push(i2 * 8 + j2);
      temp[i][j] = temp[i2][j2];
      temp[i2][j2] = tb;
    }
  }
...
function kingMoves(
  ...
  temp
	  ...
	  if(castling[turn?1:3]&&empty[i][j+1]&&empty[i][j+2]&&board[i][j+3].toLowerCase()=="r"){
		tb=temp[i][j+2];
		  temp[i][j+2]=temp[i][j];
		  temp[i][j]="-";
		  if(!main||!checkForCheck(flatten(temp), turn)){
  ...
  if(castling[turn?0:2]){
	  if(empty[i][j-1]&&empty[i][j-2]&&empty[i][j-3]&&board[i][j-4].toLowerCase()=="r"){
		tb=temp[i][j-3];
		  temp[i][j-3]=temp[i][j];
		  temp[i][j]="-";
		  if(!main||!checkForCheck(flatten(temp), turn)){
...
export function checkForCheck(board: any, turn: boolean) {
  ...
  let myPieces = boardMatrix.map((x) =>
    x.map((y: any) =>
      (turn ? y.toUpperCase() : y.toLowerCase()) == y && y != "-" ? true : false
    )
  );
